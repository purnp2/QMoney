...
public enum StockQuoteServiceFactory {
  ...
  public StockQuotesService getService(String provider, RestTemplate restTemplate) {
    if (provider == null) {
      System.out.println("the programm hasn't received any specific service provider so the default provider is used.");
      return new AlphavantageService(restTemplate);
    }

    System.out.println("Input Provider: " + provider);
